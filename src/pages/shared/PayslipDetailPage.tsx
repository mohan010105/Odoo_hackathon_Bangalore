...
export function PayslipDetailPage({ payslipId }: { payslipId: string }) {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
          ...
          actions={
            <div className="flex flex-wrap gap-2">
              <Button variant="outline" size="sm" onClick={() => window.print()}>
                <Printer aria-hidden="true" className="mr-1.5 size-3.5" /> Print
              </Button>
              <Button size="sm" onClick={() => window.print()}>
                <Download aria-hidden="true" className="mr-1.5 size-3.5" /> Download
              </Button>
            </div>
          }
        />
      ...
      <Card className="border-border/80 shadow-card print:border-0 print:shadow-none bg-card">
        <CardContent className="space-y-6 p-6 sm:p-8">
          <header className="flex flex-wrap items-start justify-between gap-4 border-b border-border/80 pb-5">
            <div>
              <p className="font-display text-xl font-bold tracking-tight text-foreground">
                DAYFLOW
              </p>
              <p className="text-xs text-muted-foreground">Enterprise Human Resource Management System</p>
            </div>
            <div className="text-right">
              <p className="font-display text-base font-bold text-foreground">{periodLabel(period)}</p>
          ...
          <dl className="grid grid-cols-2 gap-3.5 rounded-lg border border-border/80 bg-muted/20 p-4 sm:grid-cols-4">
          ...
          <p className="border-t border-border/80 pt-3 text-[11px] text-muted-foreground leading-relaxed">
            This payslip is generated by Dayflow HRMS from recorded attendance, approved time off and the
            employee's active salary structure. Amounts are in {row.currency}.
          </p>
        </CardContent>
      </Card>
